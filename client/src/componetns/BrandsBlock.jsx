...
const BrandsBlock = () => {
  ...
  React.useEffect(() => {
    if (brandId) {
      axios.get(`http://localhost:3001/api/brand/${brandId}`)
          .then((res) => {
                          ...
                          {brands.slice(0, 3).map((brand) => 
                            <div key={brand.name} className="slide-companies-block__image">
                              <div className={isAuth && adminMode ? 'slide-companies-block__actions' : 'slide-companies-block__actions_hidden'}>
                                  ...
                                  <svg className='update-brand' onClick={() => setBrandId(brand.id)} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512">
