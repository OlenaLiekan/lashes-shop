...
const ReviewItem = ({ productId }) => {
    ...
    React.useEffect(() => {
        if (productId) {
            axios.get(`http://localhost:3001/api/rating?productId=${productId}`)
                .then((res) => {
    ...
    return (
    <>
    {productRatings ? productRatings.map((rating) => 
        <div key={rating.userId} className={styles.item}>
            <div className={styles.top}>
                <div className={styles.userName}>{rating.userId}</div>
                <div className={styles.date}>{rating.createdAt.slice(0,19).replace('T', ' / ')}</div>
            </div>
